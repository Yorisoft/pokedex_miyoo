...
int PokedexDB::openDB()
{
    ...
    if (rc != SQLITE_OK)
    {
        std::cout << "SQLite could not open file! SQLite Error: " << sqlite3_errmsg(db)
                  << std::endl;
        sqlite3_close(db);
    }
    return rc;
...
void PokedexDB::closeDB(sqlite3 *db) { sqlite3_close(db); }

const std::string PokedexDB::getDBpath() { return isTestMode ? TEST_DB_PATH : MAIN_DB_PATH; }

void PokedexDB::enableTestMode(bool isTest)
{ // Used for changing directory paths between testing and live environments
    isTestMode = isTest;
}
...
//        SQLstatement.replace(pos, std::string("${language_id}").length(),
//        std::to_string(languageVersion)); pos = SQLstatement.find("${language_id}");
//    }
//
...
//    while (pos != std::string::npos) {
//        SQLstatement.replace(pos, std::string("${generation_id}").length(),
//        std::to_string(generationID)); pos = SQLstatement.find("${generation_id}");
//    }
//
...
//    while (pos != std::string::npos) {
//        SQLstatement.replace(pos, std::string("${region_id}").length(),
//        std::to_string(regionVersion)); pos = SQLstatement.find("${region_id}");
//    }
//
...
//    while (pos != std::string::npos) {
//        SQLstatement.replace(pos, std::string("${versionGroup_id}").length(),
//        std::to_string(versionGroupID)); pos = SQLstatement.find("${versionGroup_id}");
//    }
//
...
//    while (pos != std::string::npos) {
//        SQLstatement.replace(pos, std::string("${pokemon_identifier}").length(),
//        pokemonIdentifier); pos = SQLstatement.find("${pokemon_identifier}");
//    }
//
...
//    while (pos != std::string::npos) {
//        SQLstatement.replace(pos, std::string("${evoChain_id}").length(),
//        std::to_string(evoChainID)); pos = SQLstatement.find("${evoChain_id}");
//    }
//
...
//
//    // execute sql querry
//    if (sqlite3_exec(db, SQLstatement.c_str(), callback, results, &zErrMsg) != SQLITE_OK || rc !=
//    SQLITE_OK) {
...
void PokedexDB::setLanguageID(int version_id) { languageID = version_id; }

void PokedexDB::setVersionID(int version_id) { versionID = version_id; }

void PokedexDB::setGenerationID(int gen_id) { generationID = gen_id; }
void PokedexDB::setVersionGroupID(int group_version_id) { versionGroupID = group_version_id; }

void PokedexDB::setPokemonID(int pokemon_id) { pokemonID = pokemon_id; }

void PokedexDB::setMoveID(int move_ID) { moveID = move_ID; }

void PokedexDB::setEvoChainID(int evo_chain_ID) { evoChainID = evo_chain_ID; }
...
std::string PokedexDB::getPokemonIdentifier() { return pokemonIdentifier; }

void PokedexDB::setAudioID(int audio_ID) { audioID = audio_ID; }

int PokedexDB::getAudioID() { return audioID; }
