...
{
    char* zErrMsg = 0;
    SDL_Surface* textListSurface = NULL, * screen = NULL;
    SDL_Texture* textListTexture = NULL, * texture = NULL;
    ...
    screen = SDL_CreateRGBSurface(0, WINDOW_WIDTH, WINDOW_HEIGHT, DEPTH, 0, 0, 0, 0);
    ...
    while (!quit) {
        ...
        if (currentView == View::LIST) {
            ...
            for (int i = selectedItem; i < results.size() && i < selectedItem + WINDOW_HEIGHT / 24; i++) { // Adjust the 30 according to your font height
                ...
                textStream << right << setw(4) << results[i][0] << " " << results[i][1];                     // setw(4) assumes Pokemon ID is up to 3 digits.
            ...
            for (int i = 0; i < lines.size(); i++) {
                ...
                SDL_BlitSurface(lineSurface, NULL, screen, &lineRect);
        ...
        string typePath2;
        cout << "About to load " << results[selectedItem][1] << endl;
        ...
        pokemonRect.w = (WINDOW_WIDTH - maxListTextWidth); // Adjust the width to fill the space to the left of the list
        ...
        // Copy the texture to the rendering context
        SDL_BlitSurface(typeImage1, NULL, screen, &typeRect1);
        SDL_BlitSurface(pokemonImage, NULL, screen, &pokemonRect);
        ...
        if (results[selectedItem][4] != " ") {
            ...
            SDL_BlitSurface(typeImage2, NULL, screen, &typeRect2);
